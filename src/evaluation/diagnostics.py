...
Consumes per-quintile backtest results, regime classifier output, and config.
Pure consumer — no portfolio logic, no backtest re-runs.

Writes:
  outputs/charts/
    cumulative_returns.png   — multi-line cumulative excess return overlay
    drawdown.png             — primary cutoff drawdown curve
    regime_timeline.png      — regime labels + stress window shading
    rolling_ir.png           — rolling Sharpe + IR (window from config)

  outputs/csv/
    summary_stats.csv        — IR, Sharpe, MaxDD, turnover, hit rate per quintile + benchmark row
    monthly_returns.csv      — date x (portfolio_qX, benchmark, allocation, regime)
    regime_stats.csv         — per-regime risk/return decomp (each quintile + benchmark)
    subperiod_stats.csv      — by sub-period decomposition
    exposure_decomp.csv      — sector + rating exposure per quintile
...
def generate_diagnostics(
    ...
    Args:
        results: dict mapping each sensitivity quintile (e.g. 0.10) to its
            backtest output dict (from `run_backtest`).
        regime_out: output of `classify_regimes` (for regime timeline plot).
        cfg: full config dict. Uses `signal.primary_quintile` and the
    ...
    primary_q = cfg["signal"]["primary_quintile"]
    if primary_q not in results:
        log.warning(f"primary quintile {primary_q} not in results; using {sorted(results)[0]}")
    ...
    log.info(f"Generating diagnostics for {len(results)} quintile variants -> {output_dir}")

    # --- Compute layer ---
    summary_rows = {q: _compute_summary_stats(bt) for q, bt in results.items()}
    summary_df   = pd.DataFrame(summary_rows).T.rename_axis("quintile")
    ...
    exposure_df   = pd.DataFrame(exposure_rows).T.rename_axis("quintile")

    rolling_df = _compute_rolling(primary_bt, rolling_window)
    ...
    _plot_drawdown(primary_bt["portfolio_returns"], charts_dir / "drawdown.png")
    ...
    _plot_rolling(rolling_df, rolling_window, charts_dir / "rolling_ir.png")
    log.info(f"  wrote {charts_dir / 'rolling_ir.png'}")

    # --- Headline log line ---
    p = summary_rows[primary_q]
    log.info(
        f"Diagnostics complete. Primary (q={primary_q}): "
...
def _compute_summary_stats(bt: dict) -> dict:
    """Per-quintile summary metrics. Returns are already excess (vs. duration-matched Treasury)."""
...
def _compute_regime_stats(results: dict, primary_bt: dict) -> pd.DataFrame:
    ...
    One row per (series, regime). Series = each quintile + 'benchmark'.
    Regime labels are aligned with each month's actual classification from
    `regime_labels`; benchmark uses the primary quintile's regime series
    (identical across quintiles).
    """
    ...
    for q, bt in sorted(results.items()):
        ...
        for regime in ("risk_on", "neutral", "risk_off"):
            ...
            sub  = pf[mask]
            rows.append(_regime_row(f"q{int(q*100)}", regime, sub))
...
def _compute_subperiod_stats(results: dict, sub_periods: list) -> pd.DataFrame:
    """For each (quintile, sub-period), compute summary stats."""
    rows = []
    ...
    if not rows:
        return pd.DataFrame()
    return pd.DataFrame(rows).set_index(["quintile", "period"])
...
def _compute_rolling(bt: dict, window: int) -> pd.DataFrame:
    """Rolling Sharpe + IR over `window` months, annualized."""
    pf = bt["portfolio_returns"]
    ac = bt["active_returns"]

    rolling_sharpe = (pf.rolling(window).mean() / pf.rolling(window).std()) * np.sqrt(12)
    rolling_ir     = (ac.rolling(window).mean() / ac.rolling(window).std()) * np.sqrt(12)
...
def _plot_cumulative(results: dict, primary_q: float, path: Path) -> None:
    ...
    ax.set_title("Cumulative Excess Return — Quintile Sensitivity Overlay")
...
def _plot_drawdown(returns: pd.Series, path: Path) -> None:
    ...
    ax.axhline(0, color="black", linewidth=0.5)
    ax.set_title("Drawdown — Primary Cutoff")
...
def _plot_rolling(rolling_df: pd.DataFrame, window: int, path: Path) -> None:
    fig, ax = plt.subplots(figsize=(11, 5))
    ax.plot(rolling_df.index, rolling_df["rolling_sharpe"].values,
            label=f"Rolling Sharpe ({window}mo)", linewidth=1.5)
    ax.plot(rolling_df.index, rolling_df["rolling_ir"].values,
            label=f"Rolling IR ({window}mo)", linewidth=1.5)
    ax.axhline(0, color="black", linewidth=0.5)
    ax.set_title(f"Rolling {window}-Month Sharpe and Information Ratio — Primary Cutoff")
    ax.set_ylabel("Annualized ratio")
    ax.legend(loc="best")
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(path, dpi=150)
...
def _write_monthly_returns(results: dict, path: Path) -> None:
    """Wide CSV: Date x (portfolio_qX for each quintile, benchmark, allocation, regime)."""
    any_bt = next(iter(results.values()))
    df = pd.DataFrame(index=any_bt["portfolio_returns"].index)
    df.index.name = "Date"

    for q, bt in sorted(results.items()):
        df[f"portfolio_q{int(q*100)}"] = bt["portfolio_returns"]

    # Benchmark / allocation / regime from the middle quintile (or smallest if single)
